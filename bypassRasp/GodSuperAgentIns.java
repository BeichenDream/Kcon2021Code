...
public class GodSuperAgentIns {
            ...
            while ((line = mapsRandomAccessFile.readLine())!=null){
                try {
                    ...
                    if (startAddress!=0&&libFile!=null&&!libFile.isEmpty()&&endAddress-startAddress!=0){
                        ...
                        if (Arrays.equals(GodSuperAgentIns.elfHeader,elfHeader)){
    ...
    private static Object getModule(Class claszz){
        try {
            return Class.class.getMethod("getModule").invoke(claszz,null);
        }catch (Throwable e){
    ...
    public static Instrumentation run32(Map<String, Long> memoryLibraryMap,boolean bypassAgentSec){
        ...
        if (Java_java_io_RandomAccessFile_length!=0&&mmap!=0){
            RandomAccessFile randomAccessFile = null;
            try {
...
//                };
                ...
                //run
                long _JPLISAgent=unsafe.allocateMemory(1024*10);//10k _JPLISAgent
                unsafe.putInt(_JPLISAgent
                                +4 //JavaVM *
                        ,(int) jvmtiEnv);//mNormalEnvironment

                unsafe.putInt(_JPLISAgent
                                +4 //JavaVM *
                                + (3 * 4) //Structural alignment
                        ,(int)jvmtiEnv);//mRetransformEnvironment
                Class<?> instrumentClazz = Class.forName("sun.instrument.InstrumentationImpl");
                Object systemModule = getModule(instrumentClazz);
                Object oldModule = getModule(GodSuperAgentIns.class);
                if (systemModule!=null){
                    unsafe.getAndSetObject(GodSuperAgentIns.class,unsafe.objectFieldOffset(Class.class.getDeclaredField("module")),systemModule);
                }
                Constructor instrumentClazzConstructor=instrumentClazz.getDeclaredConstructor(long.class,boolean.class,boolean.class);
                instrumentClazzConstructor.setAccessible(true);
                Instrumentation instrumentation = (Instrumentation) instrumentClazzConstructor.newInstance(_JPLISAgent,true,true);
                if (systemModule!=null){
                    unsafe.getAndSetObject(GodSuperAgentIns.class,unsafe.objectFieldOffset(Class.class.getDeclaredField("module")),oldModule);
                }
    ...
    public static Instrumentation run64(Map<String, Long> memoryLibraryMap,boolean bypassAgentSec){
        long Java_java_io_RandomAccessFile_length = 0;
        long mmap = 0;
        Iterator libs = memoryLibraryMap.keySet().iterator();
        while (libs.hasNext()){
...
//                };
    ...
    public static void main(String[] args)throws Exception {
        ...
        if ("64".equals(System.getProperty("sun.arch.data.model"))){
            instrumentation = run64(memoryLibraryMap,true);
        }else {
            instrumentation = run32(memoryLibraryMap,true);
        }
        //  Cannot be use transformer
        System.out.println(instrumentation.getObjectSize(1));

        CtClass ctClass=ClassPool.getDefault().makeClass(GodSuperAgentIns.class.getResourceAsStream("GodSuperAgentIns.class"));
        ctClass.getDeclaredMethod("foo").insertBefore("System.out.println(\"111\");");


        instrumentation.redefineClasses(new ClassDefinition(String.class,readInputStream(String.class.getResourceAsStream("String.class"))));
        instrumentation.redefineClasses(new ClassDefinition(GodSuperAgentIns.class,ctClass.toBytecode()));
        foo();
    }
}
